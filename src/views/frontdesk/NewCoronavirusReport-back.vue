  ...
  <div class="add">
    ...
    <template v-if="showXinxi">
      ...
      <template v-if="form.identity=='student'">
        ...
        <mt-cell title="您是否正在参加顶岗实习："></mt-cell>
      ...
      <mt-cell title="1月16日后，是否去过湖北(不包括武汉市)："></mt-cell>
      <mt-radio v-model="form.arriveHuBei" :options="arriveHuBeiOptions"></mt-radio>
      <mt-cell title="1月16日后，是否到过武汉："></mt-cell>
      ...
      <template
        v-if="form.arriveHuBei=='true' || form.arriveWuHan=='true'||form.comefromHuBei=='true' || form.comefromWuHan=='true' ||form.touchHuBeiPerson=='true'"
      >
        ...
        <mt-cell title="到达柳州的时间："></mt-cell>
        <mt-field placeholder="请输入" v-model="form.arriveLiuZhou" type="date"></mt-field>
      </template>
      <template v-else></template>
      <template
        v-if="form.arriveHuBei=='true' || form.arriveWuHan=='true'||form.comefromHuBei=='true' || form.comefromWuHan=='true'"
      >
      ...
      <template
        v-if="form.arriveHuBei=='true' || form.arriveWuHan=='true'||form.comefromHuBei=='true' || form.comefromWuHan=='true'"
      >
        <mt-cell title="离开湖北的时间："></mt-cell>
        <mt-field  placeholder="请输入" v-model="form.leaveHubei" type="date"></mt-field>
        <mt-cell title="离开湖北的方式：车次/航班/汽车/自驾："></mt-cell>
        <mt-field
          id="jsCell"
          placeholder="请输入"
          type="textarea"
    ...
    export default {
        ...
        methods: {
            initForm(){//初始化表单
                ...
                this.$set(this.form,'tel',''); //手机号码//是否来自湖北(不包括武汉市)
                ...
                this.$set(this.form,'arriveWuHan',''); //1月16日后，是否到过武汉
            ...
            changeIdentity(val){//当改变学生或教师身份时.val为当前的值
                if(this.form.name!=''){//这里仅通过名字判断，该用户是否已经从服务端获取了自己的信息
